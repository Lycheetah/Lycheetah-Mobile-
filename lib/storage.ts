...
// Grants bonus messages (from ad watch) without exceeding cap
const FREE_TIER_BONUS_PER_AD = 5;
export async function grantFreeTierBonus(): Promise<number> {
  const today = new Date().toISOString().split('T')[0];
  const current = await getFreeTierCount();
  const newCount = Math.max(0, current - FREE_TIER_BONUS_PER_AD);
  await AsyncStorage.setItem(FREE_TIER_KEY, JSON.stringify({ date: today, count: newCount }));
  return newCount;
}

...
import { getSeasonalColor } from '../../lib/seasonal-color';
import {
...
export default function CustomizeScreen() {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {/* Seasonal suggestion */}
      {(() => {
        const seasonal = getSeasonalColor(true); // true = southern hemisphere (NZ)
        const isActive = accentColor === seasonal.color;
        return (
          <TouchableOpacity
            onPress={() => { setAccentColor(seasonal.color); saveAccentColor(seasonal.color); }}
            style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 10, padding: 10, borderRadius: 10, borderWidth: 1, borderColor: seasonal.color + (isActive ? 'AA' : '33'), backgroundColor: seasonal.color + (isActive ? '18' : '0A') }}
            activeOpacity={0.8}
          >
            <View style={{ width: 24, height: 24, borderRadius: 12, backgroundColor: seasonal.color, borderWidth: isActive ? 2 : 0, borderColor: '#fff' }} />
            <View style={{ flex: 1 }}>
              <Text style={{ color: seasonal.color, fontSize: 11, fontWeight: '700', letterSpacing: 0.5 }}>{seasonal.name}</Text>
              <Text style={{ color: SOL_THEME.textMuted, fontSize: 10 }}>{seasonal.description}</Text>
            </View>
            <Text style={{ color: seasonal.color + '88', fontSize: 9, fontFamily: Platform.OS === 'ios' ? 'Courier New' : 'monospace', letterSpacing: 1 }}>SEASONAL</Text>
          </TouchableOpacity>
        );
      })()}
